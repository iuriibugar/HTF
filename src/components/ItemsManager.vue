    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold">{{ listTitle }}</h2>
          <span class="text-sm text-gray-500">Всього: {{ items.length }}</span>
        </div>
        
        <!-- Немає елементів -->
        <div v-if="items.length === 0" class="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
          </svg>
          <p class="text-gray-600">{{ emptyMessage }}</p>
          <p class="text-gray-500 text-sm">Додайте перший елемент через форму →</p>
        </div>
        ...
        <div v-else class="space-y-4 max-h-[600px] overflow-y-auto pr-2">
          <div 
            v-for="item in items" 
            :key="item.id"
            class="bg-white border-2 rounded-lg p-4 hover:shadow-lg transition cursor-pointer"
            :class="editingId === item.id ? 'border-blue-500 shadow-lg' : 'border-gray-200'"
            ...
            <slot name="item-card" :item="item">
              ...
              <div v-else class="mb-3 h-32 bg-gray-100 rounded-lg flex items-center justify-center">
              ...
              <a v-if="item.link" :href="item.link" target="_blank" class="text-xs text-blue-600 hover:underline block mb-3 truncate">
                🔗 {{ item.link }}
              </a>
            </slot>
            ...
            <div class="flex gap-2 mt-3">
              <button 
                ...
                class="flex-1 px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm rounded transition">
                ✏️ Редагувати
              </button>
              <button 
      ...
      <div class="space-y-6 lg:sticky lg:top-4 lg:self-start">
        ...
        <div class="flex gap-4">
          <button 
            @click="$emit('save')"
            :disabled="saving"
            class="flex-1 px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
